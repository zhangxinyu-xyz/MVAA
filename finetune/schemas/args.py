...
class Args(BaseModel):
    ...
    training_type: Literal["lora", "sft", "lora_multiple_frames"] = "lora"
    ...
    video_column: Path
    data_type: str | None = None # added by xinyu
    data_shift: bool = False # added by xinyu; whether use data shift as augmentation or not
    data_shift_degree: float = 0.5
    data_select_maxframe: int | None = None # added by xinyu
    # If True, skip the full-dataset pass in prepare_dataset that fills cache/video_latent and prompt_embeddings
    skip_latent_cache_warmup: bool = False # added by xinyu

    ########## Training #########
    resume_from_checkpoint: Path | None = None
    ## added by Xinyu
    lora_weights_path: str | None = None  # Path to a LoRA checkpoint directory to load weights from (no resume)
    resume_training: bool = False  # Whether to resume optimizer/scheduler/dataloader states
    ...
    save_deepspeed_model: bool = True # added by xinyu
    ...
    def parse_args(cls):
        ...
        parser.add_argument("--data_type", type=str, default=None) # added by xinyu
        parser.add_argument("--data_shift", type=lambda x: x.lower() == 'true', default=False) # added by xinyu
        parser.add_argument("--data_shift_degree", type=float, default=0.5) # added by xinyu
        parser.add_argument("--data_select_maxframe", type=int, default=None)
        parser.add_argument(
            "--skip_latent_cache_warmup",
            action="store_true",
            help="Skip the one-time full-dataset iteration that pre-fills cache/video_latent and cache/prompt_embeddings. "
            "Use when you already precomputed these; training still loads latents from disk in each step.",
        )
        ...
        parser.add_argument("--save_deepspeed_model", type=lambda x: x.lower() == 'true', default=True)
