...
from .utils import (
    ...
    load_images_from_multiple_videos_multiple_frames,
    load_prompts,
    load_videos,
...
class BaseI2VDatasetMultipleFrameMultipleVideos(Dataset):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        train_frame_numbers, train_height, train_width = kwargs["train_resolution"]
        ...
        if image_column is not None and (data_root / image_column).exists():
            logger.info(
                "I2V multi-video dataset: loading image paths from image_column file: %s",
                data_root / image_column,
            )
            self.images = load_images(data_root / image_column, data_dir=data_root)
        elif len(self.videos) == 1:
            logger.info(
                "I2V multi-video dataset: single video mode — extracting frames via load_images_from_one_videos_multiple_frames",
            )
        ...
        elif image_column is None and (data_root / self.videos[0].parent / self.videos[-1].stem).is_dir() and any(
            (data_root / self.videos[0].parent / self.videos[-1].stem).iterdir()
        ):
            # Consistent with `utils.load_images_from_multiple_videos_multiple_frames`:
            # frame directory is `first_video.parent / video.stem`.
            logger.info(
                "I2V multi-video dataset: using pre-extracted frame folders under data_root (gate: %s)",
                data_root / self.videos[0].parent / self.videos[-1].stem,
            )
            self.images = []
            for video_path in self.videos:
                frame_dir = data_root / self.videos[0].parent / video_path.stem
                temp_images = load_images_from_folder(frame_dir)
                self.images.append(temp_images)
        else:
            logger.warning(
                "I2V multi-video dataset: pre-extracted frame folder gate failed — falling back to "
                "load_images_from_multiple_videos_multiple_frames (slow). "
                "Gate requires: image_column unset, len(videos)!=1, and non-empty dir "
                "data_root/first_video.parent/last_video.stem == %s. ",
                (data_root / self.videos[0].parent / self.videos[-1].stem) if self.videos else "N/A",
            )
            self.images = load_images_from_multiple_videos_multiple_frames(
                [data_root / v for v in self.videos], max_num_frames=train_frame_numbers
            )
            # assert True, "There is no this formats for the image load."
            
        self.frame_number = min([len(_index) for _index in self.images])
        ...
        # Check if all video files exist (paths are relative to data_root)
        ...
        if any(not path[0].is_file() for path in self.images):
            raise ValueError(
    ...
    def __get_shift_insert_index__(self, ori_index, max_increment = 4):
        """
        For an ascending list `L`, randomly add an offset to each element such that:
        `new_L[i] = L[i] + δ[i]`,
        where the `δ` sequence is non-decreasing (so `new_L` stays sorted) and
        `new_L[-1]` does not exceed `maxframe`.

        Args:
            L: An ascending list of numbers.
            maxframe: The maximum value in the new list cannot exceed `maxframe`.
            max_increment: Upper bound for each random increment
                (each increment is sampled from `[0, max_increment]`).
        ...
        N = len(ori_index)
        # Total available offset to ensure the last element does not exceed `maxframe`.
        total_available = self.frame_number - ori_index[-1]

        # Generate `N` random increments (each sampled from `[0, max_increment]`).
        increments = [random.uniform(0, max_increment) for _ in range(N)]
        
        # Compute cumulative offsets:
        #   δ[0] = increments[0]
        #   δ[i] = δ[i-1] + increments[i]
        cumulative = [0] * N
        cumulative[0] = increments[0]
        for i in range(1, N):
            cumulative[i] = cumulative[i-1] + increments[i]
        
        # If the cumulative sum exceeds `total_available`, scale it down uniformly.
    ...
    def __len__(self) -> int:
        ...
        #### for multiple videos, this should be the number of videos, otherwise this will be out of index
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        video = self.trainer.args.data_root / self.videos[index]
        image = [self.images[index][select_index] for select_index in selected_frame_index_for_insert]
        ...
        if encoded_video_path.exists():
            ...
            image = [self.preprocess(None, self.images[index][_index])[1] for _index in selected_frame_index_for_insert]
...
class I2VDatasetMultipleFrameMultipleVideosWithResize(BaseI2VDatasetMultipleFrameMultipleVideos):
    """
...
class I2VDatasetWithBuckets(BaseI2VDatasetMultipleFrameMultipleVideos):
    def __init__(
        self,
