...
def customized_recursive_step(
    ...
    recur_noise = randn_tensor(model_output.shape, generator=generator, device=model_output.device, dtype=model_output.dtype)
    beta_recur_noise = alpha_prod_t / alpha_prod_t_prev
    resur_sample = beta_recur_noise ** (0.5) * prev_sample + (1 - beta_recur_noise) ** (0.5) * recur_noise
        
    if not return_dict:
        return (resur_sample, )

    return resur_sample, prev_sample, pred_original_sample
